    ...
    <div class="page">
            ...
            <div class="simplebox">
           	 ...
           	 <ul class="list-menu">
							...
							%>
             	<li><a href="GetUpdate.asp?action=Client&sType=View&cid=<%=rs("cId")%>">      <%if rs("cUser")<>"" then%>业务员 : <%=rs("cUser")%>　<%end if%>  <%if rs("cdate")<>"" then%>录入时间 : <%=EasyCrm.FormatDate(rs("cdate"),2)%>　<%end if%>                      <b><%=rs("cCompany")%></b>        </a></li>
							<%
							rs.MoveNext
							Loop
							else
