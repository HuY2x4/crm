...
<input type="button" class="reset-button" value="日志" onclick="location.href='?action=Client&sType=View&otype=History&cid=<%=cid%>'" style="cursor:pointer" />
	...
	function CheckInput()
	{
        ...
        if (<%=Must_Records_rVisitTime%>=="1"){if(document.all.rVisitTime.value == ""){alert("<%=L_Records_rVisitTime & alert04%>");document.all.rVisitTime.focus();return false;}}
	...
	-->
				...
				<form name="Save" action="?action=Client&sType=View&otype=RecordsAddsave" method="post" onSubmit="return CheckInput();">
                    ...
                    <div class="form-line">
						<label class="st-label"><%if Must_Records_rVisitTime = 1 then %><font color="#FF0000">*</font> <%end if%> <%=L_Records_rVisitTime%></label>
						<input name="rVisitTime" type="date" id="Date1" class="Wdate" size="22" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})" />
                    </div>
			...
			<%elseif otype="RecordsEdit" then%>
				...
				<form name="Save" action="?action=Client&sType=View&otype=RecordsEditsave" method="post" onSubmit="return CheckInput();">
                    ...
                    <div class="form-line">
                     ...
                     <div class="form-line">
						<label class="st-label"><%if Must_Records_rVisitTime = 1 then %><font color="#FF0000">*</font> <%end if%> <%=L_Records_rVisitime%></label>
						<input name="rVisitTime" type="date" id="Date2" class="Wdate" size="22" value="<%=EasyCrm.FormatDate(EasyCrm.getNewItem("Records","rID",""&ID&"","rVisitTime"),2)%>" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})" />
                    </div>
