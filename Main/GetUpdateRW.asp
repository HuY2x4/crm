...
%>
	...
	function CheckInput()
	{
        ...
        if (<%=Must_Records_rVisitTime%>=="1"){if(document.all.rVisitTime.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning',content: '<%=L_Records_rVisitTime & alert04%>'});document.all.rVisitTime.focus();return false;}}
					...
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						...
						<col width="80" /><col /><col width="80" />
							...
							<tr>
								...
								<td class="td_l_r title"> <%if Must_Records_rVisitTime = 1 then %><font color="#FF0000">*</font><%end if%><%=L_Records_rVisitTime%></td>
                                <td class="td_r_l" ><input name="rVisitTime" type="text" class="Wdate" id="rVisitTime" size="22" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})"></td>
                                <td class="td_l_r title" style="display:none;"> <%if Must_Records_rState = 1 then %><font color="#FF0000">*</font> <%end if%> <%=L_Records_rState%></td>
								<td class="td_r_l" style="display:none;"><% = EasyCrm.getSelect("SelectData","Select_Type","rState","") %>&nbsp;<% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_Type_InfoAdd()' style="cursor:pointer"><script>function Select_Type_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_Type', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%> <span class="info_help help01" >&nbsp;同步客户类型</span></td>
							</tr>
							<tr> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	elseif sType="SaveAdd" then '保存添加
        ...
        rVisitTime = Request.Form("rVisitTime")
					...
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						...
						<col width="80" /><col /><col width="80" />
							...
							<tr>
								...
								<td class="td_l_r title"> <%if Must_Records_rVisitTime = 1 then %> <%end if%><%=L_Records_rVisitTime%></td>
                                <td class="td_r_l" ><input name="rVisitTime" type="text" class="Wdate" id="Text1" size="22" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})" value="<%=EasyCrm.FormatDate(EasyCrm.getNewItem("Records","rID",""&ID&"","rVisitTime"),1)%>"></td>
                                <td class="td_l_r title" style="display:none;"> <%if Must_Records_rState = 1 then %><font color="#FF0000">*</font> <%end if%> <%=L_Records_rState%></td>
								<td class="td_r_l" style="display:none;"><% = EasyCrm.getSelect("SelectData","Select_Type","rState",""&EasyCrm.getNewItem("Records","rID",""&ID&"","rState")&"") %>&nbsp;<% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_Type_InfoAdd()' style="cursor:pointer"><script>function Select_Type_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_Type', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%> <span class="info_help help01" >&nbsp;同步客户类型</span></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><%if Must_Records_rLinkman = 1 then %><font color="#FF0000">*</font> <%end if%> <%=L_Records_rLinkman%></td>
								<td class="td_r_l"> 
...
Sub RecordProgress() '跟单进度
	%>
	<script language="JavaScript">
	<!-- 跟单进度必填项提示
	function CheckInput()
	{
		if(document.all.Competitor.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning',content: '<%=L_rpCompetitor & alert04%>'});document.all.Competitor.focus();return false;}
		if (document.all.Customer_Exchange.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpCustomer_Exchange & alert04%>' }); document.all.Customer_Exchange.focus(); return false; }
		if (document.all.Products_Plans.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpProducts_Plans & alert04%>' }); document.all.Products_Plans.focus(); return false; }
		if (document.all.Have_Projects.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpHave_Projects & alert04%>' }); document.all.Have_Projects.focus(); return false; }
		if (document.all.Projects_Products.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpProjects_Products & alert04%>' }); document.all.Projects_Products.focus(); return false; }
		if(document.all.Project_Status.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning',content: '<%=L_rpProject_Status & alert04%>'});document.all.Project_Status.focus();return false;}
		if (document.all.Next_Work.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpNext_Work & alert04%>' }); document.all.Next_Work.focus(); return false; }
		if (document.all.Budget.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpBudget & alert04%>' }); document.all.Budget.focus(); return false; }
		if (document.all.Offer.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpOffer & alert04%>' }); document.all.Offer.focus(); return false; }
		if (document.all.Grasp_Degree.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpGrasp_Degree & alert04%>' }); document.all.Grasp_Degree.focus(); return false; }
		if (document.all.Decision_Chain.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpDecision_Chain & alert04%>' }); document.all.Decision_Chain.focus(); return false; }
		if (document.all.Customer_Status.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpCustomer_Status & alert04%>' }); document.all.Customer_Status.focus(); return false; }
		if (document.all.Channels_Cooperation.value == "") { art.dialog({ title: 'Error', time: 1, icon: 'warning', content: '<%=L_rpChannels_Cooperation & alert04%>' }); document.all.Channels_Cooperation.focus(); return false; }
	}
	-->
	</script>
	<%
	...
	%>
			<table width="100%" border="0" cellpadding="0" cellspacing="0"> 
				<form name="Save" action="?action=RecordProgress&sType=SaveAdd" method="post" onSubmit="return CheckInput();">
				<tr>
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
						<col width="130" /><col width="210" /><col width="120" />
							<tr class="tr_t"> 
								<td class="td_l_l" COLSPAN="4"><B>新增跟单进度</B></td>
							</tr>
						<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCompetitor %></td>
								<td class="td_r_l"><input name="Competitor" type="text" class="int" id="Competitor"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCustomer_Exchange %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_Service","Customer_Exchange","") %>&nbsp;<% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_Service_InfoAdd()' style="cursor:pointer"><script>function Select_Service_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_Service', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%>
								</td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProducts_Plans %></td>
								<td class="td_r_l"><input name="Products_Plans" type="text" class="int" id="Products_Plans"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpHave_Projects %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_YN","Have_Projects","") %>&nbsp;<% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_YN_InfoAdd()' style="cursor:pointer"><script>function Select_YN_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_YN', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%>
								</td>
							</tr>
							<tr>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProjects_Products %></td>
								<td class="td_r_l"><input name="Projects_Products" type="text" class="int" id="Projects_Products"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProject_Status %></td>
								<td class="td_r_l"><input name="Project_Status" type="text" class="int" id="Project_Status"/></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpNext_Work %></td>
								<td class="td_r_l"><input name="Next_Work" type="text" class="int" id="Next_Work"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpBudget %></td>
								<td class="td_r_l"><input name="Budget" type="text" class="int" id="Budget" /></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpOffer %></td>
								<td class="td_r_l"><input name="Offer" type="text" class="int" id="Offer"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpGrasp_Degree %></td>
								<td class="td_r_l"><input name="Grasp_Degree" type="text" class="int" id="Grasp_Degree"/></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpDecision_Chain %></td>
								<td class="td_r_l"><input name="Decision_Chain" type="text" class="int" id="Decision_Chain"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCustomer_Status %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_ExpenseOUT","Customer_Status","") %>&nbsp;<% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_ExpenseOUT_InfoAdd()' style="cursor:pointer"><script>function Select_ExpenseOUT_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_ExpenseOUT', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%></td>
							</tr>
							<tr>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpChannels_Cooperation %></td>
									<td class="td_r_l" colspan="3"><input name="Channels_Cooperation" type="text" class="int" id="Channels_Cooperation"/></td>
							</tr>
						</table>
					</td>
				</tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top" class="td_n Bottom_pd "> 
			<input name="cID" type="hidden" value="<%=cID%>">
			<input name="rpUser" type="hidden" value="<%=Session("CRM_name")%>">
			<input name="rpType" type="hidden" id="cType" value="<%=EasyCrm.getNewItem("Client","cId",""&cID&"","cType")%>">
			<input type="submit" name="Submit" class="button45" value="保存">　
			<input name="Back" type="button" id="Back" class="button43" value="关闭" onClick="art.dialog.close();">
		</td>
	</tr>
	...
	elseif sType="SaveAdd" then '保存添加
		...
		rpCompetitor=Request.Form("Competitor")
		rpCustomer_Exchange=Request.Form("Customer_Exchange")
		rpChannels_Cooperation=Request.Form("Channels_Cooperation")
		rpProducts_Plans=Request.Form("Products_Plans")
		rpHave_Projects=Request.Form("Have_Projects")
		rpProjects_Products=Request.Form("Projects_Products")
		rpProject_Status=Request.Form("Project_Status")
		rpNext_Work=Request.Form("Next_Work")
		rpBudget=Request.Form("Budget")
		rpOffer=Request.Form("Offer")
		rpGrasp_Degree=Request.Form("Grasp_Degree")
		rpDecision_Chain=Request.Form("Decision_Chain")
		rpCustomer_Status=Request.Form("Customer_Status")
		cID = Request.Form("cID")
		rpUser = Request.Form("rpUser")
		rpType = Request.Form("rpType")
		

    	Set rs = Server.CreateObject("ADODB.Recordset")
    	rs.Open "Select Top 1 * From [RecordProgress]",conn,3,2
		rs.AddNew
		rs("cID") = cID

		rs("rpCompetitor")=rpCompetitor
		rs("rpCustomer_Exchange")=rpCustomer_Exchange
		rs("rpChannels_Cooperation")=rpChannels_Cooperation
		rs("rpProducts_Plans")=rpProducts_Plans
		rs("rpHave_Projects")=rpHave_Projects
		rs("rpProjects_Products")=rpProjects_Products
		rs("rpProject_Status")=rpProject_Status
		rs("rpNext_Work")=rpNext_Work
		rs("rpBudget")=rpBudget
		rs("rpOffer")=rpOffer
		rs("rpGrasp_Degree")=rpGrasp_Degree
		rs("rpDecision_Chain")=rpDecision_Chain
		rs("rpCustomer_Status")=rpCustomer_Status
	...
	Set rsid = Server.CreateObject("ADODB.Recordset")
	if Accsql = 0 then
	rsid.Open "Select top 1 rpID From [RecordProgress] order by rpID desc",conn,1,1
	elseif Accsql = 1 then
	rsid.Open "Select @@IDENTITY as rpID From [RecordProgress]",conn,1,1
	end if
	rpID=rsid("rpID")
	rsid.close
	...
	cContent = ""
	Set rsc = Server.CreateObject("ADODB.Recordset")
	rsc.Open "Select * From [CustomField] where cTable='RecordProgress' order by Id asc ",conn,1,1
	If rsc.RecordCount > 0 Then
	Do While Not rsc.BOF And Not rsc.EOF
	...
	conn.execute ("insert into CustomFieldContent(cID,rpID,cContent) values('"&cID&"','"&rpID&"','"&cContent&"')")	
	

		conn.execute("update [Client] set cLastUpdated = '"&Now()&"' where cId = "&cID&" ")
		'插入操作记录
		conn.execute ("insert into Logfile(lCid,lClass,lAction,lUser,lTime) values('"&cID&"','"&L_Records_Progress&"','"&L_insert_action_01&"','"&Session("CRM_name")&"','"&now()&"')")	
	...
	%>
			<table width="100%" border="0" cellpadding="0" cellspacing="0"> 
				<form name="Save" action="?action=RecordProgress&sType=SaveEdit" method="post" onSubmit="return CheckInput();">
				<tr>
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
						<col width="130" /><col width="210" /><col width="120" />
							<tr class="tr_t"> 
								<td class="td_l_l" COLSPAN="4"><B>修改跟单进度</B></td>
							</tr>
								<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCompetitor %></td>
								<td class="td_r_l"><input name="Competitor" type="text" class="int" id="Competitor" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpCompetitor") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCustomer_Exchange %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_Service","Customer_Exchange",""&EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpCustomer_Exchange")&"") %> <% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_Service_InfoAdd()' style="cursor:pointer"><script>function Select_Service_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_Service', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%>
								</td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProducts_Plans %></td>
								<td class="td_r_l"><input name="Products_Plans" type="text" class="int" id="Products_Plans" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpProducts_Plans") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpHave_Projects %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_YN","Have_Projects",""&EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpHave_Projects")&"") %> <% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_YN_InfoAdd()' style="cursor:pointer"><script>function Select_YN_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_YN', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%>
								</td>
							</tr>
							<tr>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProjects_Products %></td>
								<td class="td_r_l"><input name="Projects_Products" type="text" class="int" id="Projects_Products" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpProjects_Products") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpProject_Status %></td>
								<td class="td_r_l"><input name="Project_Status" type="text" class="int" id="Project_Status" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpProject_Status") %>"/></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpNext_Work %></td>
								<td class="td_r_l"><input name="Next_Work" type="text" class="int" id="Next_Work" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpNext_Work") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpBudget %></td>
								<td class="td_r_l"><input name="Budget" type="text" class="int" id="Budget" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpBudget") %>"/></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpOffer %></td>
								<td class="td_r_l"><input name="Offer" type="text" class="int" id="Offer" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpOffer") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpGrasp_Degree %></td>
								<td class="td_r_l"><input name="Grasp_Degree" type="text" class="int" id="Grasp_Degree" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpGrasp_Degree") %>"/></td>
							</tr>
							<tr> 
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpDecision_Chain %></td>
								<td class="td_r_l"><input name="Decision_Chain" type="text" class="int" id="Decision_Chain" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpDecision_Chain") %>"/></td>
								<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpCustomer_Status %></td>
								<td class="td_r_l">
									<% = EasyCrm.getSelect("SelectData","Select_ExpenseOUT","Customer_Status",""&EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpCustomer_Status")&"") %> <% If mid(Session("CRM_qx"), 6, 1) = 1 Then %><input name="Back" type="button" id="Back" class="button227" value="新增" onclick='Select_ExpenseOUT_InfoAdd()' style="cursor:pointer"><script>function Select_ExpenseOUT_InfoAdd() {$.dialog.open('../System/GetUpdate.asp?action=SelectData&sType=Add&oType=Select_ExpenseOUT', {title: '新窗口', width: 400, height: 140,fixed: true}); };</script><%end if%>
								</td>
							</tr>
							<tr>
									<td class="td_l_r title"><font color="#FF0000">*</font><%=L_rpChannels_Cooperation %></td>
									<td class="td_r_l" colspan="3"><input name="Channels_Cooperation" type="text" class="int" id="Channels_Cooperation" value="<%=EasyCrm.getNewItem("RecordProgress","cID",""&cID&"","rpChannels_Cooperation") %>"/></td>
							</tr>
						</table>
					</td>
				</tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	elseif sType="SaveEdit" then '保存修改
    	...
    	Set rs = Server.CreateObject("ADODB.Recordset")
    	rs.Open "Select Top 1 * From [RecordProgress] where cID=" & cID,conn,3,2
	...
	cContent = ""
	Set rsc = Server.CreateObject("ADODB.Recordset")
	rsc.Open "Select * From [CustomField] where cTable='RecordProgress' order by Id asc ",conn,3,1
	If rsc.RecordCount > 0 Then
	Do While Not rsc.BOF And Not rsc.EOF
	'获取所有自定义字段
	cContent = cContent & rsc("cName") &":"& Trim(Request(rsc("cName"))) &"|" 
	...
	rsc.Close
	Set rsc = Nothing
	if EasyCrm.getNewItem("CustomFieldContent","cID",""&cID&" and rpID="&rpID&" ","cContent")="0" then
	conn.execute ("insert into CustomFieldContent(cID,rpID,cContent) values('"&cID&"','"&rpID&"','"&cContent&"')")	
	else
	conn.execute ("UPDATE [CustomFieldContent] SET cContent='"&cContent&"' Where cId ="&cID&" and rpID="&rpID&" ")
	end if
	
	
	conn.execute("update [Client] set cLastUpdated = '"&Now()&"' where cId = "&cID&" ")
		'插入操作记录
		conn.execute ("insert into Logfile(lCid,lClass,lAction,lUser,lTime) values('"&cID&"','"&L_Records_Progress&"','"&L_insert_action_02&"','"&Session("CRM_name")&"','"&now()&"')")	
