		...
		<td valign="top" class="td_n pdr10 pdt10">   
            <div class="MenuboxS">
              <ul>
				...
				<li <%if otype="RecordProgress" then%>class="hover"<%end if%>><span><a href="?action=RecordProgress&otype=RecordProgress"><%=L_Records_Progress%></a></span></li>
...
Select Case action
	...
	Case "RecordstoExcel"
		Call RecordstoExcel()
	Case "RecordProgresstoExcel"
		Call RecordProgresstoExcel()
	Case "OrdertoExcel"
		Call OrdertoExcel()
	Case "HetongtoExcel"
		Call HetongtoExcel()
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
							...
							<tr>
								...
								<td class="td_r_l" colspan=3 style="padding:5px 10px;">
									...
									<input type="checkbox" name="Exportitem24" value="1" checked> <%=L_Client_cWechat%>　
									<input type="checkbox" name="Exportitem25" value="1" checked> <%=L_Client_cPosition%>　
									<input type="checkbox" name="Exportitem26" value="1" checked> <%=L_Client_cIndustry%>
...
fso.CreateFolder(filefolder) 
	...
	if mid(Exportitem,24,1)="1" then
	strLine=strLine &", cWechat as 微信号"
	end if
	if mid(Exportitem,25,1)="1" then
	strLine=strLine &", cPosition as 具体职位"
	end if
	if mid(Exportitem,26,1)="1" then
	strLine=strLine &", cIndustry as 客户行业"
	end if
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
							...
							<tr>
								...
								<td class="td_r_l" colspan=3>
                                    ...
                                    <input type="checkbox" name="Exportitem10" value="1" checked> <%=L_Records_rVisitTime%>
								</td>
							</tr>
						</table>  
						<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
...
fso.CreateFolder(filefolder) 
    ...
    if mid(Exportitem, 10, 1) = "1" then
	strLine = strLine & ", rVisitTime as 联系时间"
	end if
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						...
						<form name="searchForm" action="?Action=RecordProgresstoExcel" method="post">
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
							<col width="100" /><col width="220" /><col width="100" />
							...
							<tr>
								...
								<td class="td_r_l" colspan=3>
									<input type="checkbox" name="Exportitem1" value="1" checked> <%=L_Records_rId%>　
									<input type="checkbox" name="Exportitem2" value="1" checked> <%=L_Records_cId%>　
									<input type="checkbox" name="Exportitem3" value="1" checked> <%=L_rpCompetitor%>　
									<input type="checkbox" name="Exportitem4" value="1" checked> <%=L_rpCustomer_Exchange%>　
									<input type="checkbox" name="Exportitem5" value="1" checked> <%=L_rpChannels_Cooperation%>　
									<input type="checkbox" name="Exportitem6" value="1" checked> <%=L_rpProducts_Plans%>　
									<input type="checkbox" name="Exportitem7" value="1" checked> <%=L_rpHave_Projects%>　
									<input type="checkbox" name="Exportitem8" value="1" checked> <%=L_rpProjects_Products%>　<br />
									<input type="checkbox" name="Exportitem9" value="1" checked> <%=L_rpProject_Status%>　
									<input type="checkbox" name="Exportitem10" value="1" checked> <%=L_rpNext_Work%>　
									<input type="checkbox" name="Exportitem11" value="1" checked> <%=L_rpBudget%>　
									<input type="checkbox" name="Exportitem12" value="1" checked> <%=L_rpOffer%>　
									<input type="checkbox" name="Exportitem13" value="1" checked> <%=L_rpGrasp_Degree%>　
									<input type="checkbox" name="Exportitem14" value="1" checked> <%=L_rpDecision_Chain%>　
									<input type="checkbox" name="Exportitem15" value="1" checked> <%=L_rpCustomer_Status%>
                                    <input type="checkbox" name="Exportitem16" value="1" checked> <%=L_Records_rUser%>　　
								</td>
							</tr>
						</table>  
						<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
							<tr>
								<td>
									<input type="submit" name="Submit" class="button45" value=" <%=L_Export%> ">　
									<input type="button" class="button42" value=" 下载文件 " onClick=window.location.href="../Soft/index.asp" />　
									<input type="button" name="button" class="button43" value=" <%=L_Clear%> " onClick=window.location.href="?action=killSession&otype=RecordProgress" />　
								</td>
							</tr>
						</table>  
						</form> 
...
sub RecordProgresstoExcel()
	...
	If Session("CRM_level") < 9 Then
        sql = sql & " And rpUser In (" & arrUser & ")"
	End If


    Session("CRM_sql_Export_RecordProgress") = sql
...
fso.CreateFolder(filefolder) 
	...
	next
	if mid(Exportitem, 1, 1) = "1" then
    strLine = strLine & "rpID as 编号"
	end if
	if mid(Exportitem, 2, 1) = "1" then
	strLine = strLine & ", cID as 公司名称"
	end if
	if mid(Exportitem, 3, 1) = "1" then
    strLine = strLine & ", rpCompetitor as 竞争对手"
	end if
	if mid(Exportitem, 4, 1) = "1" then
	strLine = strLine & ", rpCustomer_Exchange as 客户交流情况"
	end if
	if mid(Exportitem, 5, 1) = "1" then
	strLine = strLine & ", rpChannels_Cooperation as 合作渠道"
	end if
	if mid(Exportitem, 6, 1) = "1" then
	strLine = strLine & ", rpProducts_Plans as 可营销产品下步计划"
	end if
	if mid(Exportitem, 7, 1) = "1" then
	strLine = strLine & ", rpHave_Projects as 今年是否有项目"
	end if
	if mid(Exportitem, 8, 1) = "1" then
	strLine = strLine & ", rpProjects_Products as 项目涉及产品"
	end if
	if mid(Exportitem, 9, 1) = "1" then
	strLine = strLine & ", rpProject_Status as 项目情况"
	end if
	if mid(Exportitem, 10, 1) = "1" then
	strLine = strLine & ", rpNext_Work as 下一步工作"
	end if
	if mid(Exportitem, 11, 1) = "1" then
	strLine = strLine & ", rpBudget as 预算万"
	end if

	if mid(Exportitem, 12, 1) = "1" then
	strLine = strLine & ", rpOffer as 报价万"
	end if
	if mid(Exportitem, 13, 1) = "1" then
	strLine = strLine & ", rpGrasp_Degree as 把握度"
	end if
	if mid(Exportitem, 14, 1) = "1" then
	strLine = strLine & ", rpDecision_Chain as 决策链"
	end if
	if mid(Exportitem, 15, 1) = "1" then
	strLine = strLine & ", rpCustomer_Status as 客户状态"
	end if
    if mid(Exportitem, 16, 1) = "1" then
	strLine = strLine & ", rpUser as 业务员"
	end if
...
dim excelfile,tbname
Dim ExcelDriver,DBExcelPath
tbname="RecordProgress"
Server.ScriptTimeOut=360000'防止超时
set rs=server.createobject("adodb.recordset")     
sql="select "&strLine&" from ["&tbname&"] where 1 = 1 "&sql&" Order By rpId desc "'根据此SQL语句导出至Excel where 1 = 1 "&sql&"
rs.Open sql,conn,3,3
for Createtablei=0 to rs.Fields.Count-1                 '这里是为了创建sheet1用的
Createtable=Createtable&rs.fields(Createtablei).name&" text ,"
next
...
Session("CRM_sql_Export_RecordProgress") = ""

if ""&YNalert&"" = 1 then
	Response.Write ("<script>alert("""&L_Export_alert&""");</script>")
end if
	Response.Write ("<script>location.href='?action=RecordProgress&otype=RecordProgress' ;</script>")
end Sub
					...
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
							...
							<tr>
								...
								<td class="td_r_l" style="border-top:0;"> <input name="TimeBegin" type="text" id="TimeBegin" class="Wdate" style="width:100px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" />&nbsp;~&nbsp;<input name="TimeEnd" type="text" id="TimeEnd" class="Wdate" style="width:100px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" /></td>
...
sub ExpensetoExcel()
	...
	If eTimeBegin <> "" and  eTimeEnd <> "" Then
	    sql = sql & " And eDate >= '" & eTimeBegin & "' And eDate <= '" & eTimeEnd & "' "
	End If
	If eTimeBegin <> "" and  eTimeEnd = "" Then
	    sql = sql & " And eDate = '" & eTimeBegin & "' "
	End If
	else
	If eTimeBegin <> "" and eTimeEnd <> "" Then
	    sql = sql & " And eDate >= #" & eTimeBegin & "# And eDate <= #" & eTimeEnd & "# "
