...
If subAction = "searchItem" Then
    ...
    cRecordProgress = EasyCrm.Searchcode(Request.Form("RecordProgress"))
    ...
    Session("Search_Client_RecordProgress") = EasyCrm.Searchcode(Request.Form("RecordProgress"))
   ...
   If cLinkman <> "" Then
    ...
    If cRecordProgress = "1" Then sql = sql & " And cID in ( select cId from [RecordProgress] ) "
...
If cCompany = "" And cLinkman = ""  And cMobile = "" And cTel = "" And cAddress = "" And cArea = "" And cSquare = ""  And cType = "" And cTrade = "" And cStrade = "" And cSource = "" And cUser = "" And cGroup = "" And cstart = "" And cTimeBegin = "" And cTimeEnd = "" And cETimeBegin = "" And cETimeEnd = "" And cRecords = "" And cOrder = "" And cHetong = "" And cService = "" And cFile = "" And cRecordProgress = "" And SearchRange = "" Then
...
If subAction = "killSession" Then
    ...
    Session("Search_Client_RecordProgress") = ""
						...
						<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
								...
								<tr>
									...
									<td class="td_r_l" colspan=3><% = EasyCrm.getSelect("SelectData","Select_Source","Source",Session("Search_Client_cSource")) %></td>

                                    <td class="td_l_r title" style="display:none;">查询范围</td>
								...
								<tr>
									...
									<td class="td_r_l" colspan=3> 
                                        ...
                                        <input type="checkbox" name="RecordProgress" value="1" <%if Session("Search_Client_RecordProgress") = "1" then%>checked<%end if%> > 有跟单进度　
									</td>
								</tr>
								<tr>
					...
					<td valign="top" colspan=2 style="padding:10px 10px 0;" class="td_n">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_1">
							<tr class="tr_b">
								...
								<%end if%>
                                <%if Client_cIndustry = 1 then%>
								<td width="80" class="td_l_c"><%=L_Client_cIndustry%></td>
								<%end if%>
...
%>
                            ...
                            '这里按需求改动，由原来的dataPageSize（10）改为1000
							intPageSize = 1000
						...
						if Session("Search_Client_SearchRange") = "0" then
							...
							<tr class="tr">
								...
								<%if Client_cDate = 1 then%>
								<td class="td_l_c"><%=EasyCrm.FormatDate(rs("cDate"),2)%></td>
								<%end if%>
								<%if Client_cCompany = 1 then%>
							<%if Session("Search_Client_SearchRange") = "0" or Session("CRM_level")<9 then%>
								<td class="td_l_l">
								<% if inStr(""&arrUser&"",rs("cUser"))>0 or inStr(""&rs("cShareRange")&"",Session("CRM_name"))>0 then %>
									<a onclick='Client_InfoView<%=rs("cId")%>()' style="cursor:pointer"><%=rs("cCompany")%></a>
								<%else%>
									<%=rs("cCompany")%>
								<%end if%>
									<%if Client_cShare = 1 then%><%if rs("cShare") = 1 then%><span class="info_share" title="已共享">&nbsp;</span><%end if%><%end if%><%If EasyCrm.getCountItem("File","fId","fIdstr"," and cID="&rs("cID")&" ") > 0 Then%><span class="info_file" title="有附件">&nbsp;</span><%end if%>  <%if rs("cYn") = 0 then%> <span class="info_recycle" title="公海">&nbsp;</span><%end if%>
								</td>
							<%else%>
								<td class="td_l_l"><a onclick='Client_InfoView<%=rs("cId")%>()' style="cursor:pointer"><%=rs("cCompany")%></a><%if Client_cShare = 1 then%><%if rs("cShare") = 1 then%><span class="info_share" title="已共享">&nbsp;</span><%end if%><%end if%><%If EasyCrm.getCountItem("File","fId","fIdstr"," and cID="&rs("cID")&" ") > 0 Then%><span class="info_file" title="有附件">&nbsp;</span><%end if%>  <%if rs("cYn") = 0 then%> <span class="info_recycle" title="公海">&nbsp;</span><%end if%></td>
							<%end if%>
								<%end if%>
								<%if Client_cArea = 1 then%>
								...
								<%end if%>
                                <%if Client_cIndustry = 1 then%>
								<td class="td_l_c"><%=rs("cIndustry")%></td>
								<%end if%>
								<%if Client_cLinkman = 1 then%>
...
'Response.Write i '--词语数量			
							...
							<%else%>
								<td class="td_l_c"><% If mid(Session("CRM_qx"), 18, 1) = 1 Then %><input type="button" class="button_info_edit" value=" " title="<%=L_Edit%>"  onclick='Client_InfoEdit<%=rs("cId")%>()' style="cursor:pointer" /> <%end if%><% If mid(Session("CRM_qx"), 19, 1) = 1 Then %><input type="button" class="button_info_del" value=" " title="<%=L_Del%>" onclick='Client_InfoDel<%=rs("cId")%>()' style="cursor:pointer" /><%end if%></td>
							<%end if%>
							</tr>
                            
							<script>function Client_InfoView<%=rs("cId")%>() {$.dialog.open('GetUpdate.asp?action=Client&sType=InfoView&cId=<%=rs("cId")%><%if rs("cYn") = 0 then%>&YNRange=0<%end if%>', {title: '查看', width: 900,height: 580, fixed: true}); };//根据需求将弹出框拉长由480至580</script>
