    ...
    <div id="columns">
        ...
        <ul id="column3" class="column">
            ...
            <li class="widget color-8">  
                <div class="widget-head">
                    <h3>生日提醒 <span class='remind'><%if Accsql = 1 then%><%=EasyCrm.getCountItem("Linkmans","lid","lid"," and month(lBirthday) = month("&Nowdate&"()) and day(lBirthday) = day("&Nowdate&"()) ")%><%else%><%=EasyCrm.getCountItem("Linkmans","lid","lid"," and lBirthday = #"&date()&"# ")%><%end if%></span>　</h3>
                </div>
                <div class="widget-content">
					<ul>
					<%
					...
					rs.Open "select * from Linkmans where month(lBirthday) = month("&Nowdate&"()) and day(lBirthday) = day("&Nowdate&"())",conn,3,1
