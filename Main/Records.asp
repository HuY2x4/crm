...
%>
							...
							IF PN=1 THEN
							rs.Open "Select top "&intPageSize&" * From [Records] where cID<>'' "&sql&" Order By rVisitTime Desc",conn,1,1
							ELSE
							rs.Open "Select top "&intPageSize&" * From [Records] where cID<>'' "&sql&" and rId < ( SELECT Min(rVisitTime) FROM ( SELECT TOP "&pagenum&" rId FROM [Records]  where cID<>'' "&sql&" ORDER BY rVisitTime desc ) AS T ) Order By rVisitTime Desc ",conn,1,1
							END IF
							Set Rsstr=conn.Execute("Select count(rVisitTime) As RecordSum From [Records] where cID<>'' "&sql&" ",1,1)
